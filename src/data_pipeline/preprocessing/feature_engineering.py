...
class CreditCardFeatureEngineer:
    ...
    def extract_base_reward_rate(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "universal_cashback_percent" in df.columns:
            df["cashback_rate"] = pd.to_numeric(
                df["universal_cashback_percent"], errors="coerce"
            ).fillna(df["base_reward_rate"])
        else:
            df["cashback_rate"] = df["base_reward_rate"]

        logger.info(
            f"Extracted base reward rates (mean: {df['base_reward_rate'].mean():.2f}%)"
        )
        return df
    ...
    def calculate_effective_annual_fee(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["effective_fee_year1"] = np.where(
            is_waived, -credits_value, df["effective_annual_fee"]  # Negative = profit!
        )
    ...
    def filter_active_cards(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "discontinued" in df.columns:
            ...
            logger.info(
                f"Found {discontinued_count} discontinued cards out of {len(df)}"
            )
        else:
            df["is_active"] = 1
    ...
    def create_issuer_network_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "annual_fee" in df.columns:
            ...
            df["is_mid_tier"] = (
                (df["annual_fee"] >= 95) & (df["annual_fee"] < 450)
            ).astype(int)
...
class TransactionFeatureEngineer:
    ...
    def analyze_mcc_patterns(
        ...
        avg_per_mcc = (
            ...
            .apply(
                lambda x: (
                    x.groupby(mcc_col)[amount_col].mean().mean() if len(x) > 0 else 0
                )
            )
            .reset_index()
        )
