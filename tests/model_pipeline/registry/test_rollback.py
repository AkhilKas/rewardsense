...
def test_rollback_success(mock_log, mock_start, mock_set, mock_search, mock_exp, mock_registry):
    ...
    with patch.object(manager.notifier, "notify") as mock_notify:
        ...
        mock_notify.assert_any_call("Initiating rollback for personalization. Reason: Bad validation metrics", level="CRITICAL")
        mock_notify.assert_any_call("Rollback successful. Model personalization reverted to prev_id.", level="INFO")
...
def test_rollback_no_previous_version(mock_search, mock_exp, mock_registry):
    ...
    with patch.object(manager.notifier, "notify") as mock_notify:
        ...
        mock_notify.assert_any_call("Rollback failed: Could not determine previous version.", level="CRITICAL")
